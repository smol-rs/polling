...
impl Poller {
    ...
    pub(crate) fn supports_level(&self) -> bool {
        true
    }
    ...
    pub(crate) fn delete(&self, socket: BorrowedSocket<'_>) -> io::Result<()> {
        let span = tracing::trace_span!(
            "remove",
            handle = ?self.port,
            sock = ?socket,
    ...
    pub(crate) fn post(&self, packet: CompletionPacket) -> io::Result<()> {
