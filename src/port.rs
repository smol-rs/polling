...
use rustix::io::port::{self, Event as Pevent};
use rustix::io::{fcntl_getfd, fcntl_setfd, FdFlags, PollFlags};
...
impl Poller {
    ...
    pub fn delete(&self, fd: RawFd) -> io::Result<()> {
        if let Err(e) = unsafe { port::port_dissociate_fd(&self.port_fd, fd) } {
            match e {
    ...
    pub fn wait(&self, events: &mut Events, timeout: Option<Duration>) -> io::Result<()> {
        ...
        if let Err(e) = res {
            match e {
                rustix::io::Errno::TIME => return Ok(()),
                _ => return Err(e.into()),
            }
        }
...
impl Events {
    ...
    pub fn iter(&self) -> impl Iterator<Item = Event> + '_ {
        self.list.iter().map(|ev| Event {
            key: ev.userdata() as _,
            readable: ev.events() & read_flags().bits() as i32 != 0,
            writable: ev.events() & write_flags().bits() as i32 != 0,
        })
    }
}
